...
def example_signal_propagation():
    """Simulate signal propagation with auxin relay."""
    strain_library = create_strain_library()
    
    # Create model
    model = SpatialMultiStrainModel(grid_size=(40, 80), dx=0.1)
    ...
    model.add_strain(strain_library['alpha->alpha'])  # Relay: alpha-factor self-activation
    model.add_strain(strain_library['alpha->venus'])  # Receiver: alpha-factor to Venus
    
    # Place strains
    # Sender at left
    model.place_strain(0, 20, 5, radius=4, concentration=10.0)
    
    # Relays in the middle - creating a "wire"
    for i in range(5):
        pos = 15 + i*10
        model.place_strain(1, 20, pos, radius=3, concentration=5.0)
    
    # Receiver at right
    model.place_strain(2, 20, 70, radius=5, concentration=8.0)
    
    # Place initial beta-estradiol input
    model.place_molecule(BETA, 20, 5, radius=5, concentration=50.0)
    
    # Set simulation time
    model.set_simulation_time(0, 15)
    
    # Run simulation
    results = model.simulate(n_time_points=75)
    
    # Plot results - spatial distribution at different time points
    time_indices = [0, 20, 40, 60, 74]  # Select time points to display
    for time_idx in time_indices:
        model.plot_spatial_results(results, time_idx=time_idx, 
                                  molecules=[ALPHA, BETA, VENUS])
    
    # Create animation of alpha factor spreading
    alpha_anim = model.create_animation(results, molecule=ALPHA)
    
    # Plot time series at specific points
    positions = [(20, 5), (20, 25), (20, 45), (20, 70)]  # Positions along the path
    model.plot_time_series(results, positions, molecules=[ALPHA, BETA, VENUS])
    
    return results, alpha_anim


def example_bistable_switch():
    """Simulate a spatial bistable switch."""
    strain_library = create_strain_library()
    ...
    # Add strains for bistable switch (similar to Figure 3 in the paper)
    model.add_strain(strain_library['alpha-|IAA'])       # alpha inhibits IAA
    model.add_strain(strain_library['IAA-|alpha'])       # IAA inhibits alpha
    model.add_strain(strain_library['alpha->GH3'])       # alpha activates GH3 (IAA degradation)
    model.add_strain(strain_library['IAA->BAR1'])        # IAA activates BAR1 (alpha degradation)
    model.add_strain(strain_library['IAA-|GFP'])         # Reporter: IAA inhibits GFP
    
    # Place strains
    # Mutual inhibition core
    model.place_strain(0, 25, 25, radius=10, concentration=5.0)  # alpha-|IAA
    model.place_strain(1, 25, 25, radius=10, concentration=5.0)  # IAA-|alpha
    
    # Signal degradation
    model.place_strain(2, 25, 25, radius=8, concentration=3.0)   # alpha->GH3
    model.place_strain(3, 25, 25, radius=8, concentration=3.0)   # IAA->BAR1
    
    # Reporter (everywhere)
    model.place_strain(4, 25, 25, radius=20, concentration=2.0)  # IAA-|GFP
    
    # Set initial conditions - start in alpha-high state
    model.place_molecule(ALPHA, 25, 25, radius=10, concentration=10.0)
    ...
    # Plot time series at specific positions
    positions = [(25, 25), (25, 15), (15, 25), (35, 35)]
    model.plot_time_series(results, positions, molecules=[ALPHA, IAA, GFP, BAR1, GH3])
    
    return results, alpha_anim, iaa_anim, gfp_anim
...
def main():
    print("Running spatial yeast multicellular model simulations...")
    
    # Example 1: Signal propagation with relay
    print("\nExample 1: Signal propagation with relay")
    results_prop, anim_prop = example_signal_propagation()
    
    # Example 2: Bistable switch
    print("\nExample 2: Bistable switch")
    results_switch, anim_alpha, anim_iaa, anim_gfp = example_bistable_switch()
    
    # Example 3: Pattern formation
    print("\nExample 3: Pattern formation")
    results_pattern, anim_pattern_alpha, anim_pattern_venus = example_pattern_formation()
    ...
    return {
        ...
        'pattern_formation': {
            ...
            'animations': {
                'alpha': anim_pattern_alpha,
                'venus': anim_pattern_venus
            }
        }
    }
