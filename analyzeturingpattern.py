...
class TuringPatternAnalyzer:
    ...
    def optimize_wire_strain_for_turing(self, model, strain_library, params, 
                                       wire_density_range=(0.05, 0.3),
                                       n_points=5,
                                       grid_size=(100, 100),
                                       simulation_time=24,
                                       progress_callback=None):
        """
        Optimize wire strain density to produce Turing patterns.
        
        Args:
            model: SpatialMultiStrainModel class
            strain_library: Dictionary of strain parameters
            params: Dictionary with reaction parameters
            wire_density_range: Range of wire strain density to test
            n_points: Number of density points to test
        ...
        """
        results = []
        densities = np.linspace(wire_density_range[0], wire_density_range[1], n_points)
        
        for i, density in enumerate(densities):
            if progress_callback:
                progress_callback(i, n_points)
            
            # Setup model with wire strains
            np.random.seed(42)  # For reproducibility
            wire_grid = np.random.random(grid_size) < density
            
            # Initialize model
            sim_model = model(grid_size=grid_size, dx=0.1)
            
            # Add main pattern-forming strains
            # Assuming activator-inhibitor system with auxin as activator, alpha as inhibitor
            sim_model.add_strain(strain_library['alpha->IAA'])  # Inhibitor producing activator
            sim_model.place_strain(0, grid_size[0]//2, grid_size[1]//2, 
                                  shape="circle", radius=5, concentration=1.0)
            
            sim_model.add_strain(strain_library['IAA->alpha'])  # Activator producing inhibitor
            sim_model.place_strain(1, grid_size[0]//2, grid_size[1]//2, 
                                  shape="circle", radius=5, concentration=1.0)
            
            # Add wire strain for auxin to enhance its effective diffusion
            sim_model.add_strain(strain_library['IAA->IAA'], initial_grid=wire_grid)
            
            # Add reporter strain
            sim_model.add_strain(strain_library['IAA->GFP'])
            sim_model.place_strain(3, 0, 0, shape="rectangle", 
                                  width=grid_size[1], height=grid_size[0], 
                                  concentration=0.5)
            
            # Set simulation parameters
            sim_model.set_simulation_time(0, simulation_time)
            
            # Run simulation
            sim_results = sim_model.simulate(n_time_points=50)
            
            # Analyze final pattern
            final_pattern = sim_results['molecule_grids']['GFP'][-1]
            
            # Calculate natural diffusion coefficients
            D1 = sim_model.diffusion_coefficients.get(params.get('activator', 'IAA'), 0.67)
            D2 = sim_model.diffusion_coefficients.get(params.get('inhibitor', 'ALPHA'), 0.15)
            
            # Estimate effective diffusion coefficient for activator (from wave front)
            wave_positions = self.track_wave_front(sim_results)
            
            diffusion_result = self.measure_effective_diffusion(wave_positions, sim_results['t'])
            effective_D1 = diffusion_result['effective_diffusion']
            
            # Set effective diffusion coefficients for verification
            params_with_effective = params.copy()
            params_with_effective['effective_D1'] = effective_D1
            params_with_effective['effective_D2'] = D2
            
            # Verify if pattern is a Turing pattern
            verification = self.verify_turing_pattern(
                sim_results, 
            ...
            wavelength_data = self.analyze_pattern_wavelength(final_pattern)
            regularity = self.pattern_regularity_metrics(final_pattern)
            
            # Calculate turing quality metric
            # Higher is better - combines pattern regularity, isotropy, and proper wavelength
            quality_metric = 0.0
            if verification['is_regular'] and verification['is_isotropic']:
                # Base quality on regularity (low CV is good)
                quality_base = max(0, 1 - 4 * regularity['coefficient_of_variation'])
                
                # Boost for matching theoretical wavelength (if available)
                if verification['theoretical_wavelength'] > 0 and verification['wavelength_ratio'] > 0:
                    wavelength_match = max(0, 1 - abs(verification['wavelength_ratio'] - 1))
                    quality_metric = quality_base * (0.5 + 0.5 * wavelength_match)
                else:
                    quality_metric = quality_base * 0.5
            
            results.append({
                'wire_density': density,
                'effective_D1': effective_D1,
                'D2': D2,
                'diffusion_ratio': effective_D1 / D2 if effective_D1 > 0 else 0,
                'is_turing_pattern': verification['is_turing_pattern'],
                'coefficient_of_variation': regularity['coefficient_of_variation'],
                'is_regular': verification['is_regular'],
                'is_isotropic': verification['is_isotropic'],
                'measured_wavelength': verification['measured_wavelength'],
                'theoretical_wavelength': verification['theoretical_wavelength'],
                'wavelength_ratio': verification['wavelength_ratio'],
                'pattern': final_pattern,
                'quality_metric': quality_metric
            })
        
        # Find optimal density
        best_idx = np.argmax([r['quality_metric'] for r in results])
        optimal_density = results[best_idx]['wire_density']
    ...
    def plot_optimization_results(self, optimization_results, figsize=(12, 10)):
        """
        Plot the results of wire strain density optimization.
        
        Args:
            optimization_results: Results from optimize_wire_strain_for_turing
            figsize: Figure size
        ...
        optimal_density = optimization_results['optimal_density']
        
        fig = plt.figure(figsize=figsize)
        grid = plt.GridSpec(3, 3, figure=fig)
        
        # Extract data for plotting
        densities = [r['wire_density'] for r in results]
        diffusion_ratios = [r['diffusion_ratio'] for r in results]
        quality_metrics = [r['quality_metric'] for r in results]
        is_turing = [r['is_turing_pattern'] for r in results]
        
        # 1. Diffusion ratio vs density
        ax1 = fig.add_subplot(grid[0, 0:2])
        ax1.plot(densities, diffusion_ratios, 'bo-')
        ax1.axhline(y=10, color='r', linestyle='--', label='Minimum ratio for Turing')
        ax1.set_title("Effective Diffusion Ratio vs Wire Strain Density")
        ax1.set_xlabel("Wire Strain Density")
        ax1.set_ylabel("Diffusion Ratio (D_act/D_inh)")
        ax1.legend()
        
        # 2. Quality metric vs density
        ax2 = fig.add_subplot(grid[0, 2])
        ax2.plot(densities, quality_metrics, 'go-')
        ax2.axvline(x=optimal_density, color='r', linestyle='--')
        ax2.set_title("Pattern Quality Metric")
        ax2.set_xlabel("Wire Strain Density")
        ax2.set_ylabel("Quality Metric")
        
        # 3. Pattern visualization for each density
        n_patterns = len(results)
        n_rows = (n_patterns + 2) // 3  # Rounded up
        
        for i, result in enumerate(results):
            row = 1 + i // 3
            col = i % 3
            
            if row < 3:  # Only show up to 6 patterns to fit in grid
                ax = fig.add_subplot(grid[row, col])
                im = ax.imshow(result['pattern'], cmap='viridis')
                
                # Add colored border based on whether it's a Turing pattern
                for spine in ax.spines.values():
                    if result['is_turing_pattern']:
                        spine.set_edgecolor('green')
                    else:
                        spine.set_edgecolor('red')
                    spine.set_linewidth(3)
                
                # Highlight optimal density
                title = f"Density = {result['wire_density']:.2f}"
                if result['wire_density'] == optimal_density:
                    title += " (Optimal)"
                    
                ax.set_title(title)
                
                # Add key metrics as text
                metrics_text = f"D ratio: {result['diffusion_ratio']:.1f}\n"
                metrics_text += f"CV: {result['coefficient_of_variation']:.2f}\n"
                metrics_text += f"Quality: {result['quality_metric']:.2f}"
...
# Analyze a pattern
pattern = results['molecule_grids']['GFP'][-1]
analysis = analyzer.verify_turing_pattern(
    results, params, 
    D1=0.67,  # Auxin diffusion
    D2=0.15,  # Alpha-factor diffusion
    molecule='GFP'
)

# Optimize wire strain density
optimization = analyzer.optimize_wire_strain_for_turing(
...
# Plot results
fig1 = analyzer.plot_pattern_analysis(pattern, params, D1=0.67, D2=0.15)
fig2 = analyzer.plot_optimization_results(optimization)

plt.show()
"""rom scipy.optimize import root
    ...
    def measure_effective_diffusion(self, results, times):
        """
        Measure effective diffusion coefficient from wave front propagation.
        
        Args:
            results: Dictionary with wave front positions
            times: Array of time points
            
        Returns:
            Effective diffusion coefficient
        """
        # Get wave front positions
        wave_positions = results
    ...
    def optimize_wire_strain_for_turing(self, model, strain_library, params, 
                                       activator_strain='IAA->IAA',
                                       inhibitor_producing_strain='alpha->IAA',
                                       activator_producing_strain='IAA->alpha',
                                       reporter_strain='IAA->GFP',
                                       wire_density_range=(0.05, 0.3),
                                       n_points=5,
                                       grid_size=(100, 100),
                                       simulation_time=24,
        ...
        Args:
            model: SpatialMultiStrainModel class (not instance)
            strain_library: Dictionary of strain parameters
            params: Dictionary with reaction parameters
            activator_strain: Key for activator wire strain in strain_library
            inhibitor_producing_strain: Key for inhibitor-producing strain
            activator_producing_strain: Key for activator-producing strain
            reporter_strain: Key for reporter strain
            wire_density_range: Range of wire strain density to test
            n_points: Number of density points to test
            grid_size: Grid dimensions
            simulation_time: Simulation time in hours
            random_seed: Seed for random number generation
            store_patterns: Whether to store full pattern arrays in results
            progress_callback: Optional callback function for progress updates
            
        Returns:
            Dictionary with optimization results including:
            - results: List of results for each density
            - optimal_density: Wire strain density that produced best pattern
            - best_result: Complete result dictionary for optimal density
        """
        results = []
        densities = np.linspace(wire_density_range[0], wire_density_range[1], n_points)
        ...
        for i, density in enumerate(densities):
            ...
            # Add main pattern-forming strains
            # Inhibitor producing activator
            sim_model.add_strain(strain_library[inhibitor_producing_strain])
            sim_model.place_strain(0, grid_size[0]//2, grid_size[1]//2, 
                                  shape="circle", radius=5, concentration=1.0)
            
            # Activator producing inhibitor
            sim_model.add_strain(strain_library[activator_producing_strain])
            sim_model.place_strain(1, grid_size[0]//2, grid_size[1]//2, 
                                  shape="circle", radius=5, concentration=1.0)
            
            # Add wire strain for auxin to enhance its effective diffusion
            sim_model.add_strain(strain_library[activator_strain], initial_grid=wire_grid)
            
            # Add reporter strain
            sim_model.add_strain(strain_library[reporter_strain])
            sim_model.place_strain(3, 0, 0, shape="rectangle", 
                                  width=grid_size[1], height=grid_size[0], 
                                  concentration=0.5)
            ...
            try:
                ...
                # Estimate effective diffusion coefficient for activator (from wave front)
                wave_positions = self.track_wave_front(sim_results, grid_size=grid_size)
                ...
                result = {
                    ...
                    'effective_D1': effective_D1,
                    'D2': D2,
                    'diffusion_ratio': effective_D1 / D2 if D1 > 0 else 0,
                    'is_turing_pattern': verification['is_turing_pattern'],
                    'coefficient_of_variation': regularity['coefficient_of_variation'],
                    'is_regular': verification['is_regular'],
                    'is_isotropic': verification['is_isotropic'],
                ...
                # Only store pattern if requested (can be memory intensive)
                if store_patterns:
                    result['pattern'] = final_pattern
                
                results.append(result)
            
            except Exception as e:
                # Log error and continue with next density
                print(f"Error simulating density {density}: {str(e)}")
                results.append({
                    'wire_density': density,
                    'error': str(e),
                    'quality_metric': 0
        ...
        # Find optimal density
        quality_metrics = [r.get('quality_metric', 0) for r in results]
        if quality_metrics:
            best_idx = np.argmax(quality_metrics)
            optimal_density = results[best_idx]['wire_density']
            best_result = results[best_idx]
        else:
    ...
    def plot_optimization_results(self, optimization_results, figsize=(12, 10)):
        ...
        Returns:
            Matplotlib figure with optimization analysis
        """
        if not optimization_results or 'results' not in optimization_results:
            raise ValueError("Invalid optimization results provided")
            
        results = optimization_results['results']
        if not results:
            raise ValueError("Empty results in optimization data")
            
        optimal_density = optimization_results.get('optimal_density')
        
        fig = plt.figure(figsize=figsize)
        grid = plt.GridSpec(3, 3, figure=fig)
        
        # Extract data for plotting
        densities = [r.get('wire_density', 0) for r in results]
        diffusion_ratios = [r.get('diffusion_ratio', 0) for r in results]
        quality_metrics = [r.get('quality_metric', 0) for r in results]
        is_turing = [r.get('is_turing_pattern', False) for r in results]
        ...
        for i, result in enumerate(results):
            ...
            if row < 3:  # Only show up to 6 patterns to fit in grid
                ...
                # Add colored border based on whether it's a Turing pattern
                for spine in ax.spines.values():
                    if result.get('is_turing_pattern', False):
                        spine.set_edgecolor('green')
                    else:
                        spine.set_edgecolor('red')
                    spine.set_linewidth(3)
                ...
                # Highlight optimal density
                title = f"Density = {result['wire_density']:.2f}"
                if optimal_density is not None and result['wire_density'] == optimal_density:
                    title += " (Optimal)"
                    
                ax.set_title(title)
                
                # Add key metrics as text
                metrics_text = f"D ratio: {result.get('diffusion_ratio', 0):.1f}\n"
                metrics_text += f"CV: {result.get('coefficient_of_variation', 0):.2f}\n"
                metrics_text += f"Quality: {result.get('quality_metric', 0):.2f}"
